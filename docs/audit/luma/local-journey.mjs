...
report.previousAttempt={file:'local-journey-insufficient-funds.json',outcome:'A distinct earlier ordinary world stopped when its three remaining Marks could not buy a four-Mark food. This run sells one actual ore. It is a new attempt, not a rollback of the earlier world.'};
try{
  check();
  ...
  walk(6,20);
  command('buy_crystal');
  for(const[item,side,qty]of[['herb','buy',3],['ore','sell',1],['food','buy',1]])action(`${side} ${qty} ${item} at the market`,()=>{assert.ok(Math.hypot(s.hero.x-6,s.hero.z-20)<4);const q=R.quote(s,item,side,qty);R.trade(s,q);return q;});
  walk(0,23);
