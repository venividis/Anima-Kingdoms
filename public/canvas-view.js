...
const tau=Math.PI*2;
export function paintBlueprint(c,b,project,x=0,z=0,yaw=0,alpha=1){c.save();c.globalAlpha=alpha;const root={x,z,yaw:0};root.yaw=yaw*180/Math.PI;for(const p of b.parts){const o=R.Creation.point(root,p),v=project(o.x,p.y,o.z),scale=project(1,0,0).x-project(0,0,0).x;c.save();c.translate(v.x,v.y);c.rotate(-o.yaw);c.fillStyle=p.color;c.strokeStyle='#d1f9ea55';c.lineWidth=1;const w=Math.max(2,p.w*scale),h=Math.max(2,(p.d*.68+p.h*.4)*scale);if(p.role==='light'){c.shadowColor=p.color;c.shadowBlur=10;}if(p.shape==='box'){c.fillRect(-w/2,-h/2,w,h);c.strokeRect(-w/2,-h/2,w,h);}else if(p.shape==='spire'){c.beginPath();c.moveTo(0,-h/2);c.lineTo(w/2,h/2);c.lineTo(-w/2,h/2);c.closePath();c.fill();}else{c.beginPath();c.ellipse(0,0,w/2,h/2,0,0,tau);if(p.shape==='ring'){c.lineWidth=2;c.strokeStyle=p.color;c.stroke();}else c.fill();}c.restore();}c.restore();}
export class CanvasWorldView{
 ...
 project(x,y,z){const px=this.canvas.width/2+(x-this.cx)*this.scale,py=this.canvas.height*.5+(z-this.cz)*this.scale*.68-y*this.scale*.65;return{x:px,y:py,visible:px>-100&&py>-100&&px<this.canvas.width+100&&py<this.canvas.height+100};}
 groundPoint(x,y){return{x:this.cx+(x-this.canvas.width/2)/this.scale,z:this.cz+(y-this.canvas.height*.5)/(this.scale*.68)};}
 ...
 if(s.mode==='world'){
  ...
  for(const e of s.creation.instances){const r=s.kingdoms.receivers.find(r=>r.id===e.id);paintBlueprint(c,e.blueprint,this.project.bind(this),e.x,e.z,e.yaw*Math.PI/180,r?.status==='dormant'&&r.kind==='bridge'?.2:r?.kind==='gate'&&r.status!=='dormant'?.2:1);if(e.cargo)dot(e.x,2,e.z,.28,'#f2c570');}
 ...
 for(const e of s.enemies)if(e.hp>0&&!e.dead){dot(e.x,.6,e.z,e.boss?1.8:.65,e.team==='amber'?'#f0c675':'#cc7973');if(e.telegraph){const p=this.project(e.x,0,e.z);c.strokeStyle='#ff8275';c.beginPath();c.ellipse(p.x,p.y,5*this.scale,3.4*this.scale,0,0,tau);c.stroke();}}
 ...
 if(this.placement){const p=this.placement;paintBlueprint(c,p.blueprint,this.project.bind(this),p.x,p.z,p.yaw*Math.PI/180,.55);}
 dot(s.hero.x,0,s.hero.z,.65,'#081b25');dot(s.hero.x,1+s.hero.y,s.hero.z,.48,'#fff1c8');const hp=this.project(s.hero.x,1+s.hero.y,s.hero.z);c.strokeStyle='#fdf4d2';c.lineWidth=2;c.beginPath();c.moveTo(hp.x,hp.y);c.lineTo(hp.x+Math.sin(s.hero.angle)*16,hp.y+Math.cos(s.hero.angle)*11);c.stroke();
 ...
 c.fillStyle='#b7d0c3';c.font='11px system-ui';c.fillText('CARTOGRAPHIC VIEW · SAME WORLD & RULES',20,canvas.height-18);
 }
 map(canvas,s){const c=canvas.getContext('2d');c.clearRect(0,0,220,220);c.fillStyle='#162f34';c.fillRect(0,0,220,220);for(const g of R.Rain.GROUND){c.beginPath();g.poly.forEach(([x,z],i)=>i?c.lineTo(110+x*1.5,115+z*1.5):c.moveTo(110+x*1.5,115+z*1.5));c.closePath();c.fillStyle='#527965';c.fill();}c.fillStyle='#ffe8b5';c.beginPath();c.arc(110+s.hero.x*1.5,115+s.hero.z*1.5,3,0,tau);c.fill();}
}
