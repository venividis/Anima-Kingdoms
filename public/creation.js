...
export function surfaces(s,exclude=null){return(s.creation?.instances||[]).filter(e=>e.id!==exclude&&e.blueprint.kind==='structure'&&K.supported(s,e.id)).flatMap(e=>e.blueprint.parts.filter(p=>p.role==='walkway').map(p=>({...point(e,p),owner:e.id})));}
export function surface(s,x,z,exclude=null){return surfaces(s,exclude).some(p=>inside(x,z,p));}
export function obstacles(s){return(s.creation?.instances||[]).filter(e=>e.blueprint.kind==='structure'&&K.blocking(s,e.id)).flatMap(e=>e.blueprint.parts.filter(p=>p.role==='solid'&&p.y-p.h/2<2.2&&p.y+p.h/2>0).map(p=>({...point(e,p),r:Math.hypot(p.w,p.d)/2,shape:'box',owner:e.id})));}
...
export function placement(s,raw,x,z,yaw,ctx){
  ...
  for(const p of def.blueprint.parts.filter(p=>['solid','walkway'].includes(p.role))){const o=point(e,p),c=Math.cos(o.yaw),v=Math.sin(o.yaw);for(const a of [-1,1])for(const b of [-1,1]){const px=o.x+a*o.hx*c+b*o.hz*v,pz=o.z-a*o.hx*v+b*o.hz*c;if(px < -55||px>55||pz < -62||pz>48)fail('Keep every rotated physical corner inside the valley bounds.');}}
