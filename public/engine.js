...
export class Renderer{
 ...
 begin(eye,target,t,ortho=false,size=null){const gl=this.gl,w=Math.max(1,Math.round((size?.[0]||innerWidth)*this.dpr)),h=Math.max(1,Math.round((size?.[1]||innerHeight)*this.dpr));if(this.canvas.width!==w||this.canvas.height!==h){this.canvas.width=w;this.canvas.height=h;}gl.viewport(0,0,w,h);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.useProgram(this.program);this.vp=multiply(perspective(ortho?.8:1.02,w/h,.15,450),lookAt(eye,target));gl.uniformMatrix4fv(this.u.vp,false,this.vp);gl.uniform3fv(this.u.eye,eye);gl.uniform1f(this.u.time,t);}
 draw(mesh,m=transform(),opts={}){const gl=this.gl;gl.bindVertexArray(mesh.vao);gl.uniformMatrix4fv(this.u.model,false,m);gl.uniform3fv(this.u.tint,opts.tint||[1,1,1]);gl.uniform1f(this.u.glow,opts.glow||0);gl.uniform1f(this.u.sway,opts.sway||0);gl.uniform1f(this.u.alpha,opts.alpha??1);gl.depthMask((opts.alpha??1)>=1);gl.drawArrays(gl.TRIANGLES,0,mesh.count);gl.depthMask(true);}
}
