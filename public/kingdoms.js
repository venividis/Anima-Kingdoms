...
export function validate(s){const k=s.kingdoms,bad=()=>fail('Malformed Anima Kingdoms connection state.');
 ...
 for(const p of k.packets){if(!exact(p,['id','link','source','target','pitch','index','start','due','cause'])||!int(p.id,1,k.serial)||!int(p.pitch,0,12)||!int(p.index,0,C.scoreLength(instance(s,p.source,'instrument').blueprint)-1)||!int(p.start,0,k.clock)||!int(p.due,k.clock,1e9)||p.due-p.start>300||!int(p.cause,1,k.serial)||!k.links.some(l=>l.id===p.link&&l.source===p.source&&l.target===p.target&&l.pitches.includes(p.pitch)))bad();}
