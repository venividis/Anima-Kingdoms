...
export function waypoint(s,actor,target,legal,bounds){
 ...
 const topology=(s.kingdoms?.topology||0)+'|'+s.mode+'|'+(s.rain.storage.bridge>=6)+'|'+s.structures.map(b=>b.id).join(',')+'|'+s.creation.instances.filter(e=>e.blueprint.kind==='structure').map(e=>e.id).join(',');let cached=routes.get(actor);
