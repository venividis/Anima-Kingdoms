...
export class WorldView {
 ...
 const boss=new Geometry();boss.cone([0,.2,0],[0,4.5,0],1.8,1.1,[.29,.34,.35],10);boss.sphere(0,4.7,0,1.2,1.2,1.15,C.pale,10,8);boss.box(0,4.8,1,.8,.16,.2,C.gold);for(const side of[-1,1]){boss.cone([side*1,3.8,0],[side*3,1.3,.2],.48,.27,[.38,.40,.36]);boss.cone([side*3,1.3,.2],[side*3.2,.2,1],.27,.6,C.stone);boss.cone([side*.6,5.3,0],[side*2.3,7,-.5],.24,.04,C.gold);}boss.ring(0,3.8,0,1.55,.08,C.gold);this.boss=r.mesh(boss);
 const house=new Geometry();house.cone([0,0,0],[0,2.7,0],2.1,2,C.pale,8);house.cone([0,2.7,0],[0,4,0],2.7,.1,[.24,.43,.43],8);house.box(0,1.1,1.9,1.1,2.2,.2,C.dark);house.box(1.1,1.7,1.65,.45,.6,.15,C.gold);this.house=r.mesh(house);
 const farm=new Geometry();farm.box(0,.16,0,3.7,.3,3.7,[.28,.26,.2]);for(let i=-1;i<2;i++)for(let j=-1;j<2;j++){farm.cone([i,.3,j],[i,.9,j],.045,.02,C.teal);farm.sphere(i,.95,j,.32,.12,.3,C.teal,6,4);}this.farm=r.mesh(farm);
 const cover=new Geometry().box(0,1.1,0,3.5,2.2,3.5,C.stone);cover.cone([0,2.2,0],[0,2.8,0],2.6,.2,C.pale,4);this.cover=r.mesh(cover);
 const sword=new Geometry().cone([0,0,0],[0,0,2],.09,.015,C.light,5);sword.box(0,0,.1,.65,.08,.1,C.gold);this.sword=r.mesh(sword);
 const disc=new Geometry().cone([0,0,0],[0,.012,0],1,1,[1,1,1],48);this.disc=r.mesh(disc);const ring=new Geometry().ring(0,0,0,1,.025,[1,1,1],48);this.ring=r.mesh(ring);
 const flag=new Geometry().cone([0,0,0],[0,3.3,0],.045,.045,C.pale);flag.quad([0,3.2,0],[1.3,2.9,0],[1.15,2.2,0],[0,2.4,0],[1,1,1]);this.flag=r.mesh(flag);
 const gate=new Geometry();for(const side of[-1,1]){gate.cone([side*1.6,0,0],[side*1.3,3.5,0],.18,.14,C.pale);gate.cone([side*1.3,3.5,0],[0,4.7,0],.14,.04,C.pale);}gate.ring(0,2.5,0,1,.025,C.gold);this.gate=r.mesh(gate);const stall=new Geometry();stall.box(6,.55,20,2.2,1.1,1.1,C.pale);stall.box(6,1.2,20,2.6,.15,1.5,C.gold);for(const x of[4.8,7.2])stall.cone([x,0,20.4],[x,2.8,20.4],.07,.07,C.stone);stall.box(6,2.75,20,2.8,.12,1.8,C.teal);this.marketStall=r.mesh(stall);
 }
 ...
 for(const n of RESOURCES){if(n.item==='ore')for(let i=0;i<3;i++)r.draw(this.crystal,transform(n.x+Math.sin(i*3)*.7,0,n.z+Math.cos(i*3)*.7,.5,.5+i*.1,.5),{glow:.1});if(n.item==='stone')r.draw(this.cover,transform(n.x,0,n.z,.5,.4,.5));}
 for(const n of LANDMARKS.filter(n=>['boss','ctf','duel'].includes(n.kind)))r.draw(this.gate,transform(n.x,0,n.z));
 ...
 if(s.activity?.flags)for(const f of s.activity.flags)r.draw(this.flag,transform(f.x,f.carrier?1:0,f.z),{tint:f.team==='amber'?[1,.72,.28]:[.32,.75,1],glow:.3});
 if(s.mode==='boss'&&s.bossDefeated&&!s.cacheClaimed){r.draw(this.cover,transform(75,0,-47,.5,.4,.5),{tint:C.gold});r.draw(this.base.orb,transform(75,1.5,-47,2,2,2),{glow:1});}
 ...
 groundPoint(x,y){
  ...
  for(let i=0;i<4;i++){let p=i;for(let j=i+1;j<4;j++)if(Math.abs(rows[j][i])>Math.abs(rows[p][i]))p=j;if(Math.abs(rows[p][i])<1e-12)return null;[rows[p],rows[i]]=[rows[i],rows[p]];const n=rows[i][i];rows[i]=rows[i].map(v=>v/n);for(let j=0;j<4;j++)if(j!==i){const f=rows[j][i];rows[j]=rows[j].map((v,k)=>v-f*rows[i][k]);}}
  const unproject=z=>{const v=[x/innerWidth*2-1,1-y/innerHeight*2,z,1],p=rows.map(r=>r.slice(4).reduce((n,a,i)=>n+a*v[i],0));return p.slice(0,3).map(n=>n/p[3]);},a=unproject(-1),b=unproject(1),dy=b[1]-a[1];if(Math.abs(dy)<1e-8)return null;const t=-a[1]/dy;if(t<0)return null;return{x:a[0]+(b[0]-a[0])*t,z:a[2]+(b[2]-a[2])*t};
 }
 project(x,y,z){return project([x,y,z],this.r.vp,innerWidth,innerHeight);}
 map(canvas,s){const c=canvas.getContext('2d');c.clearRect(0,0,220,220);c.save();c.translate(110,115);const mode=s.mode==='world',scale=mode?1.5:3.2,a=mode?{x:0,z:0}:ARENAS[s.mode];c.scale(scale,scale);c.translate(-a.x,-a.z);c.fillStyle='#486d6477';if(mode){for(const g of Rain.GROUND){c.beginPath();g.poly.forEach(([x,z],i)=>i?c.lineTo(x,z):c.moveTo(x,z));c.closePath();c.fill();}c.strokeStyle=Rain.bridgeOpen(s.rain)?'#efd795':'#895b42';c.lineWidth=3;c.beginPath();c.moveTo(0,-7);c.lineTo(0,-19);c.stroke();for(const e of s.creation.instances){c.fillStyle=e.blueprint.kind==='structure'?'#75ead0':'#b69aff';c.beginPath();c.arc(e.x,e.z,1.4,0,7);c.fill();}for(const l of LANDMARKS){c.fillStyle=s.discoveries.includes(l.id)?'#e9d095':'#83a19a';c.fillRect(l.x-1,l.z-1,2,2);}}else{c.beginPath();c.arc(a.x,a.z,a.r,0,7);c.fill();}for(const e of s.enemies)if(e.hp>0){c.fillStyle=e.team==='amber'?'#ecc870':'#f17a66';c.beginPath();c.arc(e.x,e.z,1.3,0,7);c.fill();}c.translate(s.hero.x,s.hero.z);c.rotate(-s.hero.angle);c.fillStyle='#faf2d4';c.beginPath();c.moveTo(0,3);c.lineTo(-1.8,-2);c.lineTo(1.8,-2);c.fill();c.restore();}
}
