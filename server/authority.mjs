...
function genesis(now) {
  ...
  return {schemaVersion:2,rulesVersion:RULES_VERSION,realmId:randomUUID(),revision:0,createdAt:now,players:{},nodes:NODE_DEFINITIONS.map(node=>({...node,remaining:node.initial})),
    ...
    treasury,totals,agents:{},chat:[],events:[],blueprints:[]};
}
function assertState(s) {
  const corrupt=message=>fail('CORRUPT_REALM',`Realm custody refused: ${message}`,503);
  if(!object(s)||s.schemaVersion!==2||s.rulesVersion!==RULES_VERSION||!Number.isSafeInteger(s.revision)||s.revision<0||typeof s.realmId!=='string'||!object(s.players)||!object(s.agents))corrupt('invalid realm schema');
...
export class SharedRealm {
  ...
  read(){const row=this.db.prepare('SELECT state,checksum FROM realm WHERE id=1').get();if(!row||hash(row.state)!==row.checksum)fail('CORRUPT_REALM','Stored realm checksum does not match; refusing to reset custody.',503);let s;try{s=JSON.parse(row.state);}catch{fail('CORRUPT_REALM','Stored realm JSON cannot be read.',503);}if(object(s)&&!Object.hasOwn(s,'schemaVersion')){s.schemaVersion=2;if(!Object.hasOwn(s,'blueprints'))s.blueprints=[];}return assertState(s);}
  ...
  validateCredentials(s){for(const c of this.db.prepare('SELECT * FROM credentials').all())if(!s.players[c.player_id]||!['owner','agent'].includes(c.role)||c.role==='agent'&&(!s.agents[c.id]||s.agents[c.id].playerId!==c.player_id)||!/^[a-f0-9]{64}$/.test(c.token_hash))fail('CORRUPT_REALM','Credential custody references an unknown principal or grant.',503);}
  ...
  createSession(body) {
    exact(body,['name'],'Session');const name=plainText(body.name,1,32,'Name');const token='ak_'+randomBytes(32).toString('base64url');
    return this.transaction(()=>{const s=this.read();if(Object.keys(s.players).length>=500)fail('REALM_FULL','This realm has reached its 500-principal limit.',503);
      ...
      this.db.prepare('INSERT INTO credentials VALUES(?,?,?,?,?)').run(id,hash(token),playerId,'owner',expiresAt);s.revision++;this.append(s,c,'session.create',[]);this.write(s);this.presence.set(playerId,now);
      return {token,playerId,expiresAt,state:this.view(s,c)};
    });
  }
